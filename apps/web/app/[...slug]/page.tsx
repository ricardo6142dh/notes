...
function resolveCanonicalSlug(slug: string): string {
  const aliasMap = (allSlugsData as { aliasMap?: Record<string, string> }).aliasMap ?? {}
...
async function readPageData(slug: string): Promise<PageData | null> {
  return (
    await readJSON<PageData>(path.join(GENERATED_DIR, "pages", `${slug}.json`)) ??
    await readJSON<PageData>(path.join(GENERATED_DIR, "pages", slug, "index.json"))
  )
}
...
export async function generateStaticParams() {
  ...
  return [
    ...pages
      .filter((slug: string[]) => slug.join("/") !== config.homePage && slug.at(-1) !== "index")
...
export async function generateMetadata({
  ...
  const canonicalSlug = resolveCanonicalSlug(slugStr)
  const pageData = await readPageData(canonicalSlug)
  if (!pageData) return {}
  ...
  const canonical = `${config.site.baseUrl}/${canonicalSlug}`
  return {
    title,
    description,
...
export default async function NotePage({
  ...
  const jsonLd: Record<string, unknown> = {
    ...
    url: `${config.site.baseUrl}/${canonicalSlug}`,
  }
  if (frontmatter.date) {
