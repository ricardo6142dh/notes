...
async function main() {
  ...
  for (const f of files) {
    ...
    const target = f.slug.endsWith("/index") ? f.slug.slice(0, -"/index".length) : f.slug
    if (!slugByName.has(name)) slugByName.set(name, target)
  }

  const knownSlugs = new Set(files.flatMap((f) => (
  ...
  for (const { file, result } of rendered) {
    for (const linkTarget of result.links) {
      const normalized = slugifyPath(linkTarget)
  ...
  for (const file of files) {
    ...
    const lastSegment = slugParts.at(-1) === "index" ? slugParts.at(-2) : slugParts.at(-1)
    const targetSlug = file.slug.endsWith("/index") ? file.slug.slice(0, -"/index".length) : file.slug
    const candidates = [
      ...(lastSegment ? [lastSegment] : []),
      ...aliases,
    ]
    for (const alias of candidates) {
      const cleaned = slugifyPath(alias.startsWith("/") ? alias.slice(1) : alias)
      if (cleaned && cleaned !== targetSlug && !aliasMap[cleaned]) {
        aliasMap[cleaned] = targetSlug
      }
    }
  }
  ...
  const slugsData = {
    ...
    aliases: Object.keys(aliasMap).map((p) => p.split("/")),
    aliasMap,
  }
